...
| 库名 | PIP 安装命令 | 描述 |
|------|--------------|------|
| opencv | `pip install opencv-python` | 开源计算机视觉库，用于图像处理和计算机视觉任务 |
| matplotlib | `pip install matplotlib` | Python 绘图库，创建静态、动态和交互式可视化 |
| numpy | `pip install numpy` | 科学计算基础包，提供多维数组对象和各种派生对象 |
| os | (Python 内置) | 操作系统接口模块，提供文件和目录操作功能 |
| subprocess | (Python 内置) | 用于创建和管理子进程，执行外部命令 |
| pynput | `pip install pynput` | 控制和监控输入设备的库，支持键盘和鼠标监听 |
| flask | `pip install flask` | 轻量级 Web 框架，简单灵活，适合小型项目和 API |
| django | `pip install django` | 高级 Web 框架，功能完整，适合大型企业级应用 |
| fastapi | `pip install fastapi` | 现代高性能 Web 框架，基于类型提示，自动生成 API 文档 |
| ruff | `pip install ruff` | 极速的 Python 代码检查器和格式化工具 |
| BeautifulSoup | `pip install beautifulsoup4` | HTML 和 XML 解析库，用于网页数据抓取 |
| Pandas | `pip install pandas` | 数据分析库，提供高性能、易用的数据结构和分析工具 |
| PySide | `pip install PySide6` | Qt 框架的 Python 绑定，用于创建桌面应用程序 |
