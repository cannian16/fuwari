...
### 创建用户
-m 可以选择创建家目录，-s可以选择`/usr/sbin/nologin`来禁止外部登录。如果指定了shell就可以登录了
```bash
useradd [参数] <用户名> 
```
### 修改用户
...
- -aG <组名>，追加到组
- -u ，改uid
- -s [shell路径]，更改shell，或者禁止登录的shellnologin
...
passwd [用户名]
```
默认修改自己的密码，指定用户名更改别的用户

### 查看用户信息
```bash
id [用户名]
w #告诉你谁登录了，还告诉你每个人正在干什么以及系统有多忙，更详细
who #列出当前所有已经登录到系统的用户。
whoami #当前用户是谁
groups [用户名] #列出用户的用户组
```
### 切换用户终端
