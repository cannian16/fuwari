...
可以设置登录shell，zsh，bash，fish等等，为了安全可以使用`/usr/sbin/nologin`让此用户无法登录。

```text
#用户名:密码占位符:<uid>:<gid>:[备注]:[家目录]:<登录shell>
cannian:x:1000:1001::/home/cannian:/bin/bash
```
#### /etc/shadow
...
- -aG <组名或Gid>，追加到组
- -u ，改uid
- -s [shell路径]，更改shell，比如`/usr/sbin/bash`或者禁止登录的shell，`/usr/sbin/nologin`
...
### 常用的组
#### sudo
把用户加入到这个组就可以使用sudo命令，`/etc/sudoers`这个配置文件可以详细配置各个sudo组的成员有哪些命令权限
...
## 用户级环境变量
影响范围：仅对当前登录用户生效，使用source <path>生效。
- ~/.profile
- ~/.bashrc
- ~/.zshrc
### 示例
...
## 会话级
只有这个shell里生效，结束会话就没了。使用`export`指令
### 示例
```bash
# HTTP/HTTPS 代理
