...
| 操作目的 | 指令 | 关键参数/说明 |
| --- | --- | --- |
| **创建用户** | `useradd` | `-m` (创建家目录), `-s` (指定Shell) |
| **修改用户** | `usermod` | `-aG` (追加到组), `-L` (锁定), `-u` (改UID) |
| **删除用户** | `userdel` | `-r` (同时删除家目录和邮件池) |
| **设置密码** | `passwd` | `passwd <username>` |
| **查看用户** | `id` / `w` / `who` | 查看当前活跃用户或指定用户 UID/GID |

## 2. 账号安全的三大“护身符”

在 Linux 中，账号不仅是名字，更是权限的载体。

* **禁止交互登录 (Nologin)**：
将用户的 Shell 设置为 `/usr/sbin/nologin` 或 `/bin/false`。这样即便用户密码泄露，黑客也无法登录系统执行命令。
* **锁定账号 (Lock)**：
使用 `usermod -L <username>` 可以锁住用户密码，强制停止其登录权限，无需删除账号即可实现临时禁用。
* **最小权限集 (Sudoers)**：
不要把所有用户都加到 `sudo` 组里。编辑 `/etc/sudoers` 文件，可以精确控制某个用户只能运行特定的命令（例如只能运行 `rsync` 或 `systemctl restart nginx`）。

## 3. 核心配置文件解析
* **`/etc/passwd`**：用户信息库。包含用户名、UID、家目录路径和默认 Shell。
* **`/etc/shadow`**：密码加密库。只有 root 能读取，存放加密后的密码 hash 及过期策略。
* **`/etc/group`**：用户组库。定义了哪些用户属于哪个组。


---

**运维心得**：
不要在 Debian/Ubuntu 上随便使用 `root` 账号跑任何东西。始终保持一个 `sudo` 权限的普通用户用于日常管理，并为所有自动化任务（如你的 CI/CD、Linkding 等服务）创建专门的、受限的“工作用户”。

**你现在服务器上是不是还有其他没清理的测试账号？如果需要，我可以帮你写一个快速审计脚本，列出所有“拥有登录 Shell”但长期不活跃的账号，帮你清理一下环境。**
